...
当受信任的 preload 暴露 `__DSH_DESKTOP__` 时，客户端会选择桌面 carrier。它保留 `AbstractApiClient` 的 RPC 和 schema 行为，通过类型化 IPC 传输文本 API 请求与可安全承载二进制的响应，消费按 generation 管理的 Host 事件流，并暴露 Electron Main 的原生目录对话框。普通浏览器页面仍使用 Web carrier 及其 Host 原生目录选择器，fixture 页面仍使用 fixture carrier。
