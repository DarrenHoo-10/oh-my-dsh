...
每个模型行（两个适配器家族皆是）在折叠区里都有一个**支持图片**复选框：勾选会给该模型存下 `input: [text, image]`，取消则移除该字段，让提供方自己的默认生效。为视觉模型服务的自定义端点需要它；没有它，宿主会在图片发送前就拒绝。

在提供方行上方，**图片理解模型**选择器指明宿主图片中转在路由模型无法接收图片时要请来把图片描述成文本的模型。它列出目录中所有声明图片输入（或模态未知）的模型，写入 `agent-vision-model` 设置段，并可清除回退到拒绝行为。
