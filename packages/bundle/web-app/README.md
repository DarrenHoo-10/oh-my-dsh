...
The shipped persona asks the model to use the latest user message's language for natural-language output, including reasoning summaries and tool-call descriptions, unless the user requests another language. Code, commands, identifiers, paths, and quotations retain their original form. Shipped agent presets apply the same rule when their scoped persona shadows this bundle default; user-authored personas remain authoritative.

#### Token effect

The instruction adds a fixed persona suffix to every request that uses a shipped persona.

#### KV Cache effect

The suffix remains prefix-stable while the selected persona is unchanged.
