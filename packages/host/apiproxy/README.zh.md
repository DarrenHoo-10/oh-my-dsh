...
`session.fork` 将可选事件锚点映射到该锚点处或其后的首个 `turn/end`，使消息操作可包含该消息所在的完整轮次。锚点省略或超过末尾时，选择最后一个已完成轮次；若锚点已在日志中，而其所在轮次仍开放，则返回 `fork-unavailable`，不会向较早位置裁剪。响应和子会话摘要会携带 `seedLength`，它是只展示子会话内容时可见的第一个 seq。发布后的子会话会继承源会话的种子历史、cwd、日志中最新的 `ModelSelection` 及谱系。普通子会话随后加入源 Workspace；如果附加失败，`workspace-attach-failed` 会携带已发布的子会话 id，供客户端对账。`temporary: true` 则创建带 `origin: 'sidechat'` 的侧聊子会话，保留其消费方 `AgentHandle`，并跳过 Workspace 附加。`session.list`、搜索可见范围与全局 Session 列表帧都会排除该 origin，因此后续刷新列表也不会把临时子会话显示成普通对话。`session.discard` 只接受这种仍存活的临时 id，并等待对应 handle 完成释放，从而停止 Agent 并移除实时 Session。持久化提供方会忽略该 origin，因此已关闭的临时 fork 没有可恢复的持久日志。[SessionStore fork 决策](../../../.agents/notes/implemented/feature/2026-06-30-session-store-fork-api.md)记录了为何锚点要映射到该 `turn/end`。
