...
A catalog entry may declare `input: [text, image]` when the configured endpoint carries image input for that model. Absent (or an empty list, which reads the same) means text-only: image-bearing requests refuse before the transport, and the host refuses before the message is durable. A declared image modality serializes user image blocks as OpenAI-compatible `image_url` data-URL content parts, which requires the durable attachment service; without it the request refuses with `UNSUPPORTED_CONTENT`. The DeepSeek official endpoint itself is text-only, so only a custom endpoint behind this route should declare `image`.
