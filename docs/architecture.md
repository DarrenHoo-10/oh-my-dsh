...
| Model a message differently for the model than the surface | append the original as a surface event, then a `surfaceOp: replace` copy; the loop skips a message id already in the log |
