...
内置 persona 要求模型对自然语言输出使用用户最新一条消息的语言，包括推理摘要与工具调用描述；用户另行指定语言时，以用户要求为准。代码、命令、标识符、路径和引文保持原样。内置 agent preset 的作用域 persona 遮蔽该组合包默认值时也应用同一规则；用户自定义 persona 仍具有最终决定权。

#### Token 影响

该指令会为每个使用内置 persona 的请求添加固定的 persona 后缀。

#### KV Cache 影响

只要所选 persona 不变，该后缀就保持前缀稳定。
